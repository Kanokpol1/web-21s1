<template>
  <div class="book-details">
    <img :src="`/images/books/${book.isbn13}.jpg`" :alt="book.isbn13" />
    <p>{{ book.description }}</p>
    <h3>You might also like...</h3>
    <div class="related">
      <BookCard
        v-for="book in relatedBooks"
        :key="`book-detail-${book.isbn13}`"
        :book="book"
      />
    </div>
  </div>
...
import { getBook, getRelatedBooks, Book } from '@/_services/fakeapi'
...
export default class BookDetails extends Vue {
  ...
  created (): void { this.loadData(this.$route.params.isbn13) }
